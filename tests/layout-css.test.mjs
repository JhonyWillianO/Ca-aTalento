...
assert.match(blockFor('body[data-screen="stage"] .app-shell'), /width:\s*min\(1740px,\s*calc\(100%\s*-\s*64px\)\)/, "stage app shell must spread like the reference");
assert.match(blockFor(".stage-layout"), /grid-template-columns:\s*minmax\(240px,\s*260px\)\s+minmax\(720px,\s*1fr\)\s+minmax\(380px,\s*430px\)/, "desktop room must use the reference side, stage, and juror proportions");
...
assert.match(blockFor(".stage-toolbar"), /width:\s*min\(100%,\s*720px\)/, "stage toolbar controls must stay close to the stage instead of screen edges");
assert.match(blockFor(".stage-center-logo"), /width:\s*clamp\(116px,\s*10vw,\s*180px\)/, "central stage logo must stay compact like the reference");
...
assert.match(css, /@media\s*\(max-width:\s*1440px\)[\s\S]*?body\[data-screen="stage"\]\s+\.app-shell\s*\{[\s\S]*?width:\s*min\(1360px,\s*calc\(100%\s*-\s*28px\)\)/, "stage shell must not keep large desktop margins on notebook widths");
